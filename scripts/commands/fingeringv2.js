...
module.exports.onLoad = async () => {
  ...
  const imgPath = path.join(dir, "fingeringv2.png");

  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });

  if (!fs.existsSync(imgPath)) {
    const res = await axios.get("https://drive.google.com/uc?id=1DAenLldXkKReas22IGqtKmWs8Hevczo0", { responseType: "arraybuffer" }); // Clean action-based image
...
async function makeImage({ one, two }) {
  const dir = path.join(__dirname, "cache", "canvas");
  const bg = await jimp.read(path.join(dir, "fingeringv2.png"));
...
module.exports.run = async function ({ api, event }) {
  ...
  if (!mention.length) {
    return api.sendMessage("⚠️ একজনকে মেনশন করুন যাতে সে আপনার 'fingeringv2' এর শিকার হতে পারে!", threadID, messageID);
  }
